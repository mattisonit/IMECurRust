...
Windows의 현재 입력기 상태를 읽어 **텍스트 입력 커서(I-Beam)에 영문/한글/일본어 표시를 넣는 Rust 프로그램**입니다. 일반 화살표, 손가락, 대기, 크기 조절 또는 프로그램 자체 커서 상태에서는 IME를 조회하거나 배지를 표시하지 않습니다.

기존 `IMECur_improved.ahk`의 핵심 동작을 Rust와 Win32 API로 옮겼으며, 외부 Rust 크레이트 없이 동작하도록 구성했습니다.

## 1.0.2 변경 사항

- 현재 마우스가 Windows 시스템 I-Beam일 때만 IME 상태 조회와 표시 수행
...
- Chromium/Electron 등에서 입력 영역이 자체 커서를 사용하면 엄격한 I-Beam 판정에 따라 표시하지 않습니다.
...
- `GetCursorInfo`의 현재 커서가 시스템 `IDC_IBEAM`과 일치할 때만 IME 엔진을 호출합니다.
